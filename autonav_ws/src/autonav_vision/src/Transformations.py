...
class ImageTransformerConfig:
    def __init__(self):
        ...
        # Pixel Rejection and cleaned mask
...
class ImageTransformer(Node):
    ...
    def hough_transform(self, image):
        ...
        edges = cv2.Canny(gray, self.config.edge_thresholdOne, self.config.edge_thresholdTwo)
        lines = cv2.HoughLines(edges, self.config.rho_resolution, np.deg2rad(self.config.theta_resolution), self.config.line_threshold)
        return lines
    ...
    def publish_debug_image(self, img):
        ...
        img_copy = self.draw_detected_lines(img_copy, lines)
    ...
    def update_thresholds(self, detected_pixels):
            ...
            self.lower = np.clip([
                self.config.alpha*new_h + (1-self.config.alpha)*self.lower[0],
                self.config.alpha*new_s + (1-self.config.alpha)*self.lower[1],
                self.config.alpha*new_v + (1-self.config.alpha)*self.lower[2]
                ])
            
            self.upper = np.clip([
                self.config.alpha*new_h + (1-self.config.alpha)*self.upper[0],
                self.config.alpha*new_s + (1-self.config.alpha)*self.upper[1],
                self.config.alpha*new_v + (1-self.config.alpha)*self.upper[2]
                ])
    ...
    def apply_blur(self, img):
        ...
        for _ in range(self.config.blur_iterations):
            img = cv2.GaussianBlur(img, self.get_blur_level, 0)
    ...
    def apply_hsv(self, img):
        ...
        # Collect ground pixels for threshold adaptation
        detected_pixels = img[mask == 255]
        if len(detected_pixels) >= 100:
            self.update_thresholds(detected_pixels)
