...
{
public:
	Expandify() : AutoNav::Node("autonav_vision_expandifier") {}
	~Expandify() {}
	...
	void onConfigSpaceReceived(const nav_msgs::msg::OccupancyGrid::SharedPtr cfg)
	{
		...
		for (int x = 0; x < ExpandifyConstants::MAP_RES; x++)
		{
			for (int y = robotY; y < ExpandifyConstants::MAP_RES; y++)
			{
				auto idx = x + ExpandifyConstants::MAP_RES * y;
				cfg_space.at(idx) = 100;
			}
		}
